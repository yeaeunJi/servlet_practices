...
public class BoardServlet2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ("writeform".equals(action)) {
			...
			vo.setCount(++count);
			new BoardDao2().updateCount(vo);
		} else if ("updateform".equals(action)) {
			Long no = Long.parseLong(request.getParameter("no"));
			String keyword = request.getParameter("keyword");
			BoardVo vo = new BoardDao2().findOne(no);
			request.setAttribute("keyword", keyword);
			if(vo!=null) {
				request.setAttribute("vo", vo);
			...
			BoardVo vo = null;
			vo = new BoardDao2().findOne(no); 
			if (!new BoardDao2().getChildCount(vo)	) 	new BoardDao2().delete(vo);
			WebUtil.redirect(request.getContextPath()+"/board2", request, response);
